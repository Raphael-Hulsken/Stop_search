...
-createLimiPlot.cc : This code will plot the limit plots and the ratio cross section mesured and cross section excluded. It need root plots that are writen by the combine software. To understand how it works and the explaination are given in the twiki (ask others for that twiki). For know it needs 4 root file in input but it could be easily changed.
